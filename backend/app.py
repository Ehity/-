...
def scan_subscriptions(df_raw):
    """Умный алгоритм поиска подписок. На вход DataFrame, на выход DataFrame."""
    df = df_raw.copy()
    ...
    for group_key, group in df.groupby('group_key'):
        ...
        if date_col and len(group) >= 2:
            ...
            if len(dates) >= 2:
                ...
                # Диапазон месячной подписки 25-40 дней (защита от сдвигов)
...
def analyze_statement(file_path: str) -> dict:
    """Путь к файлу выписки → подписки и сводка по деньгам.

    Обёртка над scan_subscriptions: читает файл через parser.parse_statement,
    добавляет расчёт стоимости из financial_metrics и возвращает словарь,
    который ждёт test_backend.py и любой внешний код.
    """
    from financial_metrics import calculate_subscription_costs
    from parser import parse_statement

    statement = parse_statement(file_path)
    found = scan_subscriptions(statement)
    subscriptions = found.to_dict("records") if not found.empty else []

    for subscription in subscriptions:
        # scan_subscriptions отдаёт медианную сумму списания — она же средняя за период
        subscription["average_amount"] = subscription["amount"]
        subscription.update(calculate_subscription_costs(subscription))

    active = [s for s in subscriptions if s["period"] != "Нерегулярно"]
    return {
        "subscriptions": subscriptions,
        "summary": {
            "subscriptions_count": len(active),
            "total_monthly_cost": round(sum(s["monthly_cost"] for s in active), 2),
            "total_annual_cost": round(sum(s["annual_cost"] for s in active), 2),
        },
    }
